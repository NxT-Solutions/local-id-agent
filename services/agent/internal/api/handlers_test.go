...
import (
	...
	"github.com/rqc-icu/localid-agent/services/agent/internal/providers/mock"
...
func TestHandleOpenAPI_DevMode(t *testing.T) {
	s := testServer(t)
...
func TestHandleOpenAPI_Production(t *testing.T) {
	s := testServer(t)
	s.cfg.Server.DevMode = false
	server := httptest.NewServer(s.Handler())
	defer server.Close()
